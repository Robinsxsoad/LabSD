...
public class MSPastryProtocol implements Cloneable, EDProtocol {
    ...
    // public void receiveRoute(Object msg, Node srcNode) {
        ...
        case Message.MSG_SEARCH:
            o("\n[ Recibi la solicitud por la canción ]\t"+m.body+"[ Nodo = "+RoutingTable.truncateNodeId(nodeId) +" | Id mensaje = "+m.id+" | "+m.nrHops+" saltos]");
            deliver(m);
            break;    
        }
    }
    ...
    // private void route(Object msg, Node srcNode) {
        ...
        //o(String.format("[%s].Ruta( [Tipo=%s] [src:%s] [dest:%s] [m.id=%d] ): [Siguiente salto:%s]",
